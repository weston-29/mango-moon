...
/*
 * Get stored byte array of sprite bitmap at given rotation and write to relevant buffer
 * Thrust and Explode flag arguments indicate whether the relevant bitmap corresponds to thrusting or exploding, rather than just the rotation sprites
 */
...
// Draw moon at specified coordinates
void gamegl_draw_moon(int x, int y, color_t c);

/*
 * Draw lunar lander bitmap at specified angle onscreen
*/
void gamegl_draw_lander(int x, int y, int rotation, color_t c);

/*
 * Draw thrusting trail or exploding sprite bitmap at specified angle onscreen
 * Thrust and explode argument flags indicate whether effect is an explosion or a thrust
*/
 ...
 */
void gamegl_draw_line(int x1, int y1, int x2, int y2, color_t c);

// draws starry background onscreen, takes STARTSTATE or GAMESTATE to determine which scene's stars to draw
 ...
 * You can leave this function unimplemented if not doing the extension.
 */
void gamegl_draw_triangle(int x1, int y1, int x2, int y2, int x3, int y3, color_t c);
