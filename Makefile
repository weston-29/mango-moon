...
TEST_PROGRAM = testphysics_gl.bin
...
SOURCES = mymodule.c trig.c gamegl.c

all: $(PROGRAMS)

# Flags for compile and link
# ARCH 	= -march=rv64im -mabi=lp64 (old flags, see below for floating point support)
# Citation: I researched floating point flags at this link: 
ARCH = -march=rv64imfd -mabi=lp64
ASFLAGS = $(ARCH)
...
# Build and run the test program binary
test: $(TEST_PROGRAM)
	mango-run $<
